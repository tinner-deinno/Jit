...
# สร้าง system prompt จากบทบาทของ agent (ถ้ามีการกำหนด)
#
# พารามิเตอร์:
#   $1 - agent name (optional)
#   $2 - explicit system prompt (optional, from --system/-s flag)
#
# คืนค่า:
#   system prompt ที่เหมาะสมสำหรับการส่งไปยัง LLM
#
# ตรรกะการทำงาน:
#   หากมีการระบุ system prompt อย่างชัดเจน (ผ่าน --system/-s) ให้ใช้ค่านั้นโดยตรง
#   หากไม่ได้ระบุอย่างชัดเจน แต่ระบุชื่อ agent ให้ค้นหาบทบาทของ agent นั้นจาก agent_filter.sh
#   หากไม่ได้ระบุทั้ง agent และ system prompt ให้ใช้ค่า default:
#     "คุณคือผู้ช่วย AI ของระบบ มนุษย์ Agent (Jit) โดย MDES-Innova. ตอบอย่างชัดเจน มีโครงสร้าง สรุปผลเป็นภาษาไทย."
#
# หมายเหตุ: ฟังก์ชันนี้ทำงานร่วมกับ agent_filter.sh เพื่อให้ได้บทบาทของ agent
# หาก agent_filter.sh ไม่สามารถโหลดได้หรือไม่พบข้อมูล agent จะใช้ค่า default
_build_system() {
...
# ตรวจสอบว่าผู้ให้บริการหนึ่งรายพร้อมใช้งานหรือไม่
#
# พารามิเตอร์:
#   $1 - adapter - ชื่อสคริปต์ adapter ที่ใช้เรียกผู้ให้บริการนี้
#   $2 - key_env - ชื่อตัวแปรสภาพแวดล้อมที่มี API key (อาจว่าง)
#   $3 - base_url - URL พื้นฐานของผู้ให้บริการ (อาจว่าง)
#   $4 - base_url_env - ชื่อตัวแปรสภาพแวดล้อมที่มี base_url (อาจว่าง)
#   $5 - cli - คำสั่ง CLI ที่ใช้เรียกผู้ให้บริการ (อาจว่างสำหรับ HTTP transport)
#   $6 - timeout - เวลาหมดเวลาในหน่วยวินาที (อาจว่าง ใช้ค่าเริ่มต้น)
#
# คืนค่า:
#   0 หากผู้ให้บริการพร้อมใช้งาน
#   1 หากผู้ให้บริการไม่พร้อมใช้งาน
#
# ตรวจสอบ:
#   สำหรับ adapter ประเภท CLI: ตรวจสอบว่าคำสั่งมีอยู่ใน PATH และสามารถรันได้
#   สำหรับ adapter ประเภท HTTP: ตรวจสอบว่า endpoint health-check ตอบสนอง
#   จำเป็นต้องมีตัวแปรสภาพแวดล้อมที่เหมาะสม (API key, ฯลฯ) ถ้าถูกกำหนดไว้
#
# ใช้งาน:
#   เรียกจาก _attempt เพื่อตรวจสอบผู้ให้บริการแต่ละรายก่อนที่จะพยายามเรียกใช้
_provider_available() {
...
# เรียกใช้ผู้ให้บริการ LVM หนึ่งรายตามที่ระบุ
#
# พารามิเตอร์:
#   $1 - adapter - ชื่อสคริปต์ adapter ที่ใช้เรียกผู้ให้บริการนี้
#   $2 - key_env - ชื่อตัวแปรสภาพแวดล้อมที่มี API key (อาจว่าง)
...
#   $5 - cli - คำสั่ง CLI ที่ใช้เรียกผู้ให้บริการ (อาจว่างสำหรับ HTTP transport)
#   $6 - timeout - เวลาหมดเวลาในหน่วยวินาที (อาจว่าง ใช้ค่าเริ่มต้น)
#   $7 - model - model ID ที่ต้องการใช้
#   $8 - system - system prompt สำหรับการสนทนา
#   $9 - user - user prompt หรือคำถามที่ต้องการถาม
#
# คืนค่า:
#   ผลลัพธ์จากผู้ให้บริการ LLM (ข้อความที่สร้างขึ้น) ไปยัง stdout
#   คืนค่า 0 หากสำเร็จ คืนค่า 1 หากล้มเหลว
#
# กระบวนการทำงาน:
#   1. โหลดตัวแปรสภาพแวดล้อมที่จำเป็น:
#      - API key จากตัวแปรที่ระบุใน key_env (ถ้ามี)
#      - base_url จากตัวแปรที่ระบุใน base_url_env หรือใช้ค่าที่ระบุโดยตรง (ถ้ามี)
#   2. ตั้งค่าสภาพแวดล้อมสำหรับการเรียก adapter:
#      - PROVIDER_API_KEY - ค่าของ API key
#      - PROVIDER_BASE_URL - ค่าของ base_url
#      - PROVIDER_CLI - ค่าของ cli (ถ้ามี)
#      - PROVIDER_TIMEOUT - ค่าของ timeout หรือค่าเริ่มต้น 120 วินาที
#   3. เรียก adapter script ด้วยพารามิเตอร์:
#      - adapter call <model> <system> <user>
#   4. คืนผลลัพธ์จาก adapter ไปยัง stdout
#
# ข้อผิดพลาดที่อาจเกิดขึ้น:
#   - Adapter script ไม่พบหรือไม่สามารถเรียกใช้ได้
#   - ตัวแปรสภาพแวดล้อมที่จำเป็น (API key) ไม่ได้ตั้งค่า
#   - ผู้ให้บริการไม่พร้อมใช้งานหรือตอบสนองช้าเกิน timeout
#   - Adapter ส่งคืนข้อผิดพลาดไปยัง stderr
#
# หมายเหตุ: ฟังก์ชันนี้ไม่ทำการตรวจสอบว่าผู้ให้บริการพร้อมใช้งานก่อนเรียก
# การตรวจสอบความพร้อมใช้งานควรทำโดยใช้ _provider_available() ก่อนที่จะเรียกฟังก์ชันนี้
_provider_call() {
  local adapter="$1" key_env="$2" base_url="$3" base_url_env="$4" cli="$5" timeout="$6"
...
# จำกัดจำนวนการเรียกใช้ LLM ที่สามารถทำงานพร้อมกันได้ทั่วทั้งระบบ
#
# พารามิเตอร์:
#   $1 - max - จำนวนสูงสุดของการเรียกใช้ที่อนุญาตให้ทำงานพร้อมกัน (0 หรือติดลบ = ไม่จำกัด)
#   $2... - command... - คำสั่งและอาร์กิวเมนต์ที่ต้องการเรียกใช้
#
# คืนค่า:
#   คืนค่า exit code ของคำสั่งที่เรียกใช้
#
# กลไกการทำงาน:
#   หาก max <= 0 หรือไม่มีคำสั่ง flock ให้เรียกใช้คำสั่งโดยตรงโดยไม่มีการจำกัด
#   หาก max > 0 และมี flock:
#     1. สร้างไดเรกทอรีสำหรับไฟล์ล็อคถ้าจำเป็น: $JIT_LOCK_DIR/slots/
#     2. พยายามครอบครองหนึ่งในช่องล็อคที่มีอยู่ (0 ถึง max-1)
#     3. ใช้ flock -n เพื่อพยายามล็อคแบบ non-blocking บนแต่ละช่อง
#     4. หากได้ล็อคใดล็อคหนึ่ง ให้เรียกใช้คำสั่งแล้วปล่อยล็อคเมื่อเสร็จ
#     5. หากทุกช่องล็อคถูกครอบครองหมด (ทุกสล็อตว่างงานหมด):
#        - รอคอยให้มีช่องล็อคว่างโดยการบล็อกบนสล็อต 0 เป็นเวลาสูงสุด 60 วินาที
#        - หลังจากครบเวลาหมดหรือได้ล็อค ให้เรียกใช้คำสั่ง
#     6. ทำความสะอาดโดยการปิดไฟล์ตัวแทนที่เปิดทั้งหมด
#
# วัตถุประสงค์:
#   ป้องกันไม่ให้เกิดการเรียกใช้ LLM พร้อมกันมากเกินไปจนทำให้ระบบ過負荷
#   ให้การทำงานเป็นธรรม (fairness) ระหว่าง agents ต่างๆ
#   ป้องกัน resource exhaustion จากการเรียกใช้พร้อมกันมากเกินไป
#
# ตัวแปรสภาพแวดล้อมที่ใช้:
#   $JIT_LOCK_DIR - ไดเรกทอรีพื้นฐานสำหรับไฟล์ล็อค (ค่าเริ่มต้น: /tmp/manusat-lock)
#
# ตัวอย่างการใช้งาน:
#   _with_global_slot 6 jit_with_lock "llm-innova" 30 -- _attempt
#   หมายถึง: อนุญาตให้มีการเรียกใช้ LLM พร้อมกันได้สูงสุด 6 ครั้ง
#             ใช้คีย์ล็อค "llm-innova" เวลาหมดเวลาล็อค 30 วินาที
#             จากนั้นเรียกใช้ฟังก์ชัน _attempt
#
# หมายเหตุ: ฟังก์ชันนี้พึ่งพาคำสั่ง flock ซึ่งอาจไม่มีในทุกระบบ
# หาก flock ไม่มีให้ใช้งาน ฟังก์ชันนี้จะเรียกใช้คำสั่งโดยตรงโดยไม่มีการจำกัด
_with_global_slot() {
  local max="$1"; shift
...
# อ่านแถวจากตัวแปรทั่วโลก $CANDIDATES; ใช้ $SYSTEM, $USER_PROMPT, $LOG_AGENT.
#
# เดินผ่านรายการผู้ให้บริการที่จัดลำดับความสำคัญแล้ว (จาก _resolve)
# ลองเรียกผู้ให้บริการแต่ละรายตามลำดับจนกว่าจะสำเร็จหรือหมดตัวเลือก
# สำหรับแต่ละผู้ให้บริการ:
#   1. ตรวจสอบว่าผู้ให้บริการพร้อมใช้งาน (_provider_available)
#   2. หากพร้อม ให้เรียกใช้ผู้ให้บริการนั้น (_provider_call)
#   3. หากสำเร็จ ให้พิมพ์ผลลัพธ์และคืนค่า 0
#   4. หากล้มเหลว ให้ลองผู้ให้บริการถัดไปในรายการ (fallback)
#
# พารามิเตอร์:
#   ไม่มีพารามิเตอร์โดยตรง แต่ใช้อินพุตจากตัวแปรทั่วโลก:
#   $CANDIDATES - TSV จาก _resolve ที่มีรายการผู้ให้บริการเรียงลำดับ
#   $SYSTEM - system prompt สำหรับ LLM
#   $USER_PROMPT - prompt ที่ผู้ใช้ส่งมา
#   $LOG_AGENT - ชื่อ agent สำหรับการบันทึก log (หากไม่ระบุ จะใช้ "anon")
#
# คืนค่า:
#   0 หากผู้ให้บริการอย่างน้อยหนึ่งรายตอบสนองสำเร็จ
#   1 หากผู้ให้บริการทั้งหมดล้มเหลว (พิมพ์ข้อผิดพลาดไปยัง stderr)
#
# ผลข้างเคียง:
#   พิมพ์ผลลัพธ์จากผู้ให้บริการ LLM ที่สำเร็จเป็นครั้งแรกไปยัง stdout
#   บันทึกการดำเนินการไปยังระบบ log ผ่าน log_action
_attempt() {
  local tried=0 row
