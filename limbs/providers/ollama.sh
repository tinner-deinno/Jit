...
case "${1:-}" in
  available)
    ...
    # Some gateways require auth even on version; retry with bearer via stdin (not cmdline, for security).
    if printf '%s\n' "Authorization: Bearer $TOKEN" | curl -sf --max-time 6 -H @- "$BASE_URL/api/version" >/dev/null 2>&1; then
      exit 0
    fi
...
PY
    ...
    RESP="$(printf '%s\n' "Authorization: Bearer $TOKEN" | curl -s --max-time "$TIMEOUT" "$BASE_URL/api/chat" \
